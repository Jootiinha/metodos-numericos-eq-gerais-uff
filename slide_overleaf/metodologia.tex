...
\State $n \gets$ número de incógnitas ($n = (N-1)^2$)
\If{$n \leq 40000$}
...
\tiny
Fatoração: $O(n^{3/2})$, Resolução: $O(n)$, Memória: $O(n)$
\end{block}
\end{frame}
...
\begin{block}{Notação Importante}
\begin{itemize}
\item $N$: número de subintervalos em cada direção
\item $n$: número de incógnitas do sistema ($n = (N-1)^2$)
\item Exemplo: $N=192$ $\Rightarrow$ $n = 191^2 = 36.481$ incógnitas
\item Exemplo: $N=256$ $\Rightarrow$ $n = 255^2 = 65.025$ incógnitas
\end{itemize}
\end{block}
