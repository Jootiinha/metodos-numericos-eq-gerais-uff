...
\begin{block}{Norma $L^2(\Omega)$ Integral}
O erro relativo na norma $L^2(\Omega)$ é definido por:
\begin{equation*}
E_{L^2} = \frac{\|u_{\mathrm{aprox}} - u_{\mathrm{exata}}\|_{L^2(\Omega)}}{\|u_{\mathrm{exata}}\|_{L^2(\Omega)}}
\end{equation*}
onde $\|u\|_{L^2(\Omega)}^2 = \int_\Omega |u(x,y)|^2 \, dx \, dy$.
\end{block}

\begin{block}{Aproximação Discreta pela Regra do Retângulo}
Em malha uniforme com passo $h$, aproximamos a integral pela regra do retângulo:
\begin{equation*}
\|u\|_{L^2(\Omega)}^2 = \int_\Omega |u(x,y)|^2 \, dx \, dy \approx h^2 \sum_{i,j} |u_{i,j}|^2 = h^2 \|\mathbf{u}\|_2^2
\end{equation*}
Portanto: $\|u\|_{L^2(\Omega)} \approx h \|\mathbf{u}\|_2$
\end{block}
\end{frame}

\begin{frame}{Quantificação do Erro (continuação)}
\begin{block}{Erro Relativo Discreto}
Aplicando a aproximação, obtemos:
\begin{align*}
E_{L^2} &\approx \frac{h \|\mathbf{U}_{\mathrm{aprox}} - \mathbf{U}_{\mathrm{exata}}\|_2}{h \|\mathbf{U}_{\mathrm{exata}}\|_2} \\
&= \frac{\sqrt{h^2 \sum_{i,j} |U_{\mathrm{aprox},i,j} - U_{\mathrm{exata},i,j}|^2}}{\sqrt{h^2 \sum_{i,j} |U_{\mathrm{exata},i,j}|^2}}
\end{align*}
\end{block}

\begin{block}{Justificativa}
\begin{itemize}
\item A norma discreta com peso $h^2$ é uma aproximação de ordem $O(h^2)$ da norma $L^2(\Omega)$
\item Para $h \to 0$, a aproximação converge para a norma integral
\item O erro de discretização é consistente com a ordem do método (2ª ordem)
\end{itemize}
\end{block}
\end{frame}
...
\footnotesize
\begin{block}{Configuração do Grupo 1}
$\Theta = \{0, \pi/8, \pi/4\}$
\end{block}
...
\tiny
\begin{block}{Configurações dos Grupos}
\begin{itemize}
\item \textbf{Grupo 1}: $\Theta = \{0, \pi/8, \pi/4\}$
\item \textbf{Grupo 2}: $\Theta = \{\pi/16, \pi/8, 3\pi/16\}$
\item \textbf{Grupo 3}: $\Theta = \{\pi/8, 3\pi/16, \pi/4\}$
\end{itemize}
\end{block}
...
\begin{block}{Configurações}
\tiny
Grupo 1: $\Theta = \{0, \pi/8, \pi/4\}$ \quad
Grupo 2: $\Theta = \{\pi/16, \pi/8, 3\pi/16\}$ \quad
...
\begin{itemize}
\item \textbf{Grupo 1} ($\Theta = \{0, \pi/8, \pi/4\}$): Erro $1.0 \times 10^{-2}$ (melhor)
\item \textbf{Grupo 2} ($\Theta = \{\pi/16, \pi/8, 3\pi/16\}$): Erro $1.4 \times 10^{-2}$ (40\% maior)
\item \textbf{Grupo 3} ($\Theta = \{\pi/8, 3\pi/16, \pi/4\}$): Erro $1.3 \times 10^{-2}$ (30\% maior)
\end{itemize}
\end{block}
